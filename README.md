...
**Status:** pre-alpha. Active development. Latest release: [**v0.3.1**](https://github.com/leotorrealba/sprino/releases/tag/v0.3.1) — audit-export guard consolidated into the service layer, dual-adapter HTTP/MCP parity test, OCC docs precision fix, Docker-smoke CI path-filter correction.
...
- **v0.3.1** ✅ shipped — Architecture fix: audit-export entitlement guard consolidated into the service layer (single enforcement point for HTTP + MCP). Dual-adapter parity test added. OCC language in EXPLAINED.md made precise. Docker-smoke CI path filter corrected.
